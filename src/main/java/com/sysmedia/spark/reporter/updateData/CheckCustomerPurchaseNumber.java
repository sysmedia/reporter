...
public class CheckCustomerPurchaseNumber {
 ...
 //           InsertPayTypeForOneShop.insertPayType(checkList);
    ...
    public static ArrayList<String> getCheckList(ArrayList<String> shops){
        ...
        for(String shopId: shops) {
            // System.out.println("begin to handle " + shopId + " customer purchase number");

                for (String type : typeList) {
                    for (String year : yearList) {
                        //取得会员购物次数
                        String number = collection.getSingleLineInfoByType("t_shop_customer_count", "totalCount", shopId, year, type);
                        if(StringUtils.isEmptyOrWhitespaceOnly(number)) number = "0";
                        int value = Integer.parseInt(number);

                        if(value <= 100) {
                            System.out.println(shopId + ":" + year +   " type = " + type + " number is " + number);
                            //取得总的购物次数
